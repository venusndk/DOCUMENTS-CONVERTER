...
### Authentication & User Workspace (Phase 17 completion, master directive numbering)

Real accounts (`documents_converter/api/accounts.py`) — separate from,
and additive to, `auth.py`'s existing pre-shared-key access control
(`API_KEYS`). That still decides *whether a request is let in at all*;
this decides *whose workspace a request acts on*. An unauthenticated or
API-key-only caller keeps working exactly as before, with no account
attached to its jobs — accounts are opt-in, the same shape as Phase
16's AI features.

| Endpoint | Purpose |
|---|---|
| `POST /api/v1/account/signup` | Creates an account and logs it in immediately |
| `POST /api/v1/account/login` / `/logout` | — |
| `GET /api/v1/account/me` | The current session's account |
| `GET`/`PUT /api/v1/account/preferences` | A caller-defined settings blob (default target format, theme, ...) — `PUT` replaces it whole |
| `GET /api/v1/account/usage` | Job counts by status for the current account, computed live (not a second, driftable counter) |
| `GET /api/v1/account/history` | The current account's own past jobs, newest first |
| `POST /api/v1/jobs/{id}/save` / `/unsave` | Exempts one job from the normal retention-window cleanup |

**Server-side session cookies, not a JWT** — an explicit choice made
for this phase (this project's own dual audience, a browser page and
script/API callers, made either reasonable; a real login UI tipped it
toward cookies). A session is a real, database-backed row
(`UserSession`) — only a SHA-256 hash of the raw cookie value is ever
stored, the same reasoning as never storing a plaintext password.
Passwords are hashed with **argon2id** (`argon2-cffi`), OWASP's current
first recommendation, ahead of bcrypt.

**Real CSRF protection**, since a cookie is attached automatically by
the browser on every request, unlike a bearer token a script has to
attach by hand: a signed double-submit cookie. At login/signup, a
second cookie is set — `HMAC-SHA256(SESSION_SECRET, session token)` —
deliberately *not* `HttpOnly`, so same-origin JS can read it and must
echo it back as an `X-CSRF-Token` header on every mutating request;
`accounts.get_current_user` recomputes the HMAC from the session cookie
and compares. A cross-site attacker's forged request still carries the
session cookie automatically but can't read or set that header's
correct value (the Same-Origin Policy blocks reading a cookie set for
this origin from another one) — no extra server-side storage needed for
the CSRF half, it's derived, not stored.

```powershell
curl.exe -c cookies.txt -X POST http://127.0.0.1:8000/api/v1/account/signup `
  -H "Content-Type: application/json" -d '{"email":"you@example.com","password":"a real password"}'
curl.exe -b cookies.txt http://127.0.0.1:8000/api/v1/account/history
```

**Saved jobs, not "every logged-in user's job lives forever":**
`POST /api/v1/jobs/{id}/save` is an explicit, per-job opt-in —
`JobStore._cleanup_expired` exempts *only that job* from its normal
retention-window deletion, not everything a given account has ever
submitted. Keeps storage growth bounded and caller-controlled instead
of an unbounded, automatic liability of merely being logged in.

**A real migration bug found and fixed while building this phase**:
`JobRecord.saved` is a `NOT NULL` boolean added to a table this project
already has real rows in — the autogenerated Alembic migration omitted
`server_default`, which both SQLite and Postgres require to backfill an
existing row with when adding a `NOT NULL` column (a Python-side
`default=False` on the ORM model only ever applies to a future
`INSERT`). Reproduced for real: built a test database at the pre-Phase-
17 migration head, inserted a real job row, confirmed the un-fixed
migration would have nothing to backfill that row with, then added
`server_default=sa.false()` and confirmed the same database migrates
cleanly with the existing row correctly backfilled to `saved=false`.

**Honest, disclosed gaps:** Phase 16's AI-call usage isn't tracked
per-account yet (that feature's own audit trail predates accounts and
stays anonymous) — `GET /api/v1/account/usage` reports job/batch counts
only. A batch (`BatchRecord`) whose own retention window has passed is
deleted even if one of the jobs it grouped was individually `saved` —
only the *grouping* is lost in that case, the job and its result stay
directly reachable via `GET /api/v1/jobs/{id}`. The web page's account
UI covers signup/login/logout and history/save, but has no dedicated
preferences panel yet — the API itself is complete and tested
(`tests/test_accounts_api.py`); a UI for it is a natural, later,
disclosed addition, not silently skipped.

### Administration & Observability (Phase 18 completion, master directive numbering)

Every previous phase inspects or acts on *one document, one job, one
account*. Phase 18 (`documents_converter/api/admin.py`) is the first
to look at the service itself: a real admin dashboard, live queue and
provider monitoring, job analytics, real metrics, and a queryable
audit log.

| Endpoint | Purpose |
|---|---|
| `GET /admin` | The dashboard page — queue, providers, analytics, and recent audit events, refreshed every 15s |
| `GET /metrics` | Real Prometheus text exposition format — HTTP request counts/latencies, live queue depth, job counts by status |
| `GET /api/v1/admin/queue` | Real RQ/Redis introspection — queued/started/failed/scheduled/finished counts, every registered worker's real state |
| `GET /api/v1/admin/providers` | Every external dependency this project has: Tesseract, LibreOffice, Redis, the database, Gemini (configured, not a live call — see Phase 16's own quota note) |
| `GET /api/v1/admin/analytics` | System-wide job counts by status/target, success rate, average duration — computed live |
| `GET /api/v1/admin/audit-log` | This project's own audit trail (`audit.py`, Phase 11), now queryable through the API, not only stdout/a file |

**Not a new, separate admin-auth system** — an extension of Phase 17's
real accounts. `ADMIN_EMAILS` (comma-separated, empty by default, same
shape as `API_KEYS`) promotes/demotes `User.is_admin` at signup/login;
every `/api/v1/admin/*` route requires an ordinary valid session *plus*
that flag (`accounts.get_current_admin`) — `403`, not `404`, for a
logged-in non-admin, since these routes existing isn't a secret worth
hiding. `GET /admin` itself loads for anyone (like `GET /`); the real
gate is server-side, per API call its own JS makes, and a non-admin
visitor sees a clear message, not a blank page.

**Real Prometheus metrics, not a custom JSON shape** — a deliberate
choice (see the free-form design decision below) over a lighter, no-
new-dependency custom endpoint: `GET /metrics` plugs directly into real
production monitoring (Prometheus, Grafana) with zero glue code.
`documents_converter_http_requests_total`/`_duration_seconds` update
continuously via a middleware on every request, labeled by the matched
route's own path *template* (`/api/v1/jobs/{job_id}`), never the raw
path — a raw path would put a fresh, unbounded label value in
Prometheus for every distinct job id ever requested, a well-known
cardinality blowup avoided by construction, not by convention.
`documents_converter_jobs_current{status}` is a real Gauge, not a
Counter, and says so in its own `HELP` text: a job row can be deleted
by `JobStore`'s own retention cleanup, so "how many jobs are in this
status right now" can legitimately decrease, unlike a lifetime total.

**Two genuinely different kinds of number, both real**: `GET
/api/v1/admin/analytics` answers "what has this service's own data
done, historically" — a live SQL aggregation for a human looking at a
dashboard. `GET /metrics` answers "what is this process doing right
now" — continuous instrumentation for monitoring tooling. Building only
one of these would have left a real gap: a dashboard number isn't a
substitute for something Prometheus can alert on, and a raw metric
isn't a substitute for a breakdown a person can actually read.

**The audit log became real, persisted, and queryable this phase** —
`audit.log_event()` (Phase 11) has always written to stdout and,
optionally, a file; neither was ever something this service's own API
could show back to a caller. Every event now also writes one row to a
real `audit_log` table (best-effort: a database hiccup here is caught
and printed, never allowed to break the real feature the event
describes — logging that a conversion happened must never be able to
make the conversion itself fail). `GET /api/v1/admin/audit-log`
supports `limit` (capped at 1000 — this table has no automatic
retention/pruning yet, a real, disclosed limitation for a long-running
deployment) and `event` filtering.

**A real bug found and fixed while building this phase** — the exact
same class of migration bug Phase 17 found and fixed for
`JobRecord.saved`: the autogenerated migration added `User.is_admin` as
a `NOT NULL` boolean with no `server_default`, which fails against a
`users` table that already has real rows (any account created since
Phase 17 shipped). Caught and reproduced the same way — a test database
built at the pre-Phase-18 migration head, a real user row inserted,
then confirmed `server_default=sa.false()` backfills it correctly —
before this ever ran against real data.

**Job analytics' average duration comes from the audit log, not a new
column** — `job_completed` events already carry a real `duration_ms`
(logged since Phase 14); rather than adding a column to `JobRecord`
purpose-built for tracking this, `job_analytics()` reads it back out of
the most recent 1000 persisted `job_completed` audit rows. One new
table (`audit_log`) doing double duty for two of this phase's own line
items, not two separate, overlapping pieces of storage.
...
`SESSION_SECRET` (Phase 17, master directive numbering: Authentication
& User Workspace) passes through the same way. Unlike `GEMINI_API_KEY`,
leaving it unset doesn't disable a feature — accounts still work with
no configuration at all, the same local/dev default this project uses
everywhere — it just means every restart of this container invalidates
every already-logged-in browser's CSRF token (see that config value's
own docstring). Set one explicitly for anything past local/dev use:
...
Verified for real against a live stack: signed up through the real
`api` container, got back a real session cookie and a real,
non-`HttpOnly` CSRF cookie; confirmed `PUT /api/v1/account/preferences`
correctly `403`s with no `X-CSRF-Token` header and correctly `200`s
with the right one; confirmed the round-tripped preference value; and
confirmed `POST /api/v1/account/logout` really invalidates the
session (`GET /api/v1/account/me` afterward is a real `401`, not a
cached success).

`ADMIN_EMAILS` (Phase 18, master directive numbering: Administration &
Observability) is the same shape again — safe to leave unset (every
account stays a non-admin, `GET /api/v1/admin/*` stays a real `403` for
all of them), set to make the given address(es) real admins the next
time they sign up or log in:
...
Verified for real against a live stack: signed up as that email inside
the real `api` container and confirmed `GET /api/v1/account/me` reports
`is_admin: true`; confirmed every `/api/v1/admin/*` route returns real
data (not a `403`) for that session, and a real `403` for a second,
ordinary account signed up in the same container; confirmed `GET
/metrics` returns real, live Prometheus text — including this exact
verification session's own real HTTP request counts — read straight
from the running container, not a fixture.
